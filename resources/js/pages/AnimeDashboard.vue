...
import { Play, Trophy, Clock, Tv, Star, Zap, Crown, BookOpen, Library } from 'lucide-vue-next';
...
const props = defineProps({
    ...
    badges: Array,
    stuAnime: Object,
    mangaReading: { type: Array, default: () => [] },
    mangaStats: { type: Object, default: () => ({ chapters: 0, volumes: 0, completed: 0 }) },
    mangaBadges: { type: Array, default: () => [] }
})
    ...
    <AppLayout>
        <div class="p-6 space-y-8">
            ...
            <div class="flex items-center justify-end">
                <div class="flex bg-white dark:bg-gray-800 rounded-full p-1 border border-gray-200 dark:border-gray-700 shadow-sm w-fit">
                    <button 
                        @click="activeTab = 'anime'"
                        class="px-6 py-2 rounded-full text-sm font-bold transition-all duration-300"
                        :class="activeTab === 'anime' ? 'bg-indigo-600 text-white shadow-md' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700'"
                    >
                        Anime
                    </button>
                    <button 
                        ...
                        class="px-6 py-2 rounded-full text-sm font-bold transition-all duration-300"
                        :class="activeTab === 'manga' ? 'bg-red-600 text-white shadow-md' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700'"
                    >
                        Manga
                    </button>
                </div>
            ...
            <!--                 ZONE ANIME                     -->
            <!-- ============================================== -->
            <div v-show="activeTab === 'anime'" class="space-y-8 animate-in fade-in duration-500 slide-in-from-bottom-4">
                    ...
                    <Card class="hover:shadow-md transition-shadow">
                        <CardContent class="flex items-center gap-4 p-6">
                            ...
                            <div>
                                ...
                                <h3 class="text-2xl font-bold">{{ stats?.episodes || 0 }}</h3>
                            </div>
                        </CardContent>
                    </Card>
                    ...
                    <Card class="hover:shadow-md transition-shadow">
                        <CardContent class="flex items-center gap-4 p-6">
                            ...
                            <div>
                                ...
                                <h3 class="text-2xl font-bold">{{ stats?.time_spent || '0h' }}</h3>
                            </div>
                        </CardContent>
                    </Card>
                    ...
                    <Card class="hover:shadow-md transition-shadow">
                        <CardContent class="flex items-center gap-4 p-6">
                            ...
                            <div>
                                ...
                                <h3 class="text-2xl font-bold">{{ stats?.completed_count || 0 }}</h3>
                            </div>
                        </CardContent>
                    </Card>
                </div>

                <div v-if="watching && watching.length > 0">
            ...
            <!--                 ZONE MANGA                     -->
            <!-- ============================================== -->
            <div v-show="activeTab === 'manga'" class="space-y-8 animate-in fade-in duration-500 slide-in-from-bottom-4">
                
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <Card class="hover:shadow-md transition-shadow">
                        <CardContent class="flex items-center gap-4 p-6">
                            <div class="p-3 bg-red-100 dark:bg-red-900/40 rounded-full">
                                <BookOpen class="w-6 h-6 text-red-600 dark:text-red-400" />
                            </div>
                            <div>
                                <p class="text-sm font-medium text-gray-500">Chapitres lus</p>
                                <h3 class="text-2xl font-bold">{{ mangaStats?.chapters || 0 }}</h3>
                            </div>
                        </CardContent>
                    </Card>
                    ...
                    <Card class="hover:shadow-md transition-shadow">
                        <CardContent class="flex items-center gap-4 p-6">
                            <div class="p-3 bg-orange-100 dark:bg-orange-900/40 rounded-full">
                                <Library class="w-6 h-6 text-orange-600 dark:text-orange-400" />
                            </div>
                            <div>
                                <p class="text-sm font-medium text-gray-500">Volumes possédés</p>
                                <h3 class="text-2xl font-bold">{{ mangaStats?.volumes || 0 }}</h3>
                            </div>
                        </CardContent>
                    </Card>
                    ...
                    <Card class="hover:shadow-md transition-shadow">
                        <CardContent class="flex items-center gap-4 p-6">
                            ...
                            <div>
                                <p class="text-sm font-medium text-gray-500">Mangas terminés</p>
                                <h3 class="text-2xl font-bold">{{ mangaStats?.completed || 0 }}</h3>
                            </div>
                        </CardContent>
                    </Card>
                </div>

                <div v-if="mangaReading && mangaReading.length > 0">
                    <h2 class="text-xl font-bold mb-4 flex items-center gap-2 text-gray-800 dark:text-gray-100">
                        Lecture en cours
                    </h2>
                </div>
                ...
                <div v-else
                    class="text-center py-16 bg-gray-50 dark:bg-gray-800/30 rounded-2xl border-2 border-dashed border-gray-300 dark:border-gray-700">
                    <h3 class="text-lg font-bold text-gray-900 dark:text-white">Rien à lire</h3>
                    <p class="text-gray-500 mb-6">Trouve une bonne romance tranquille sans scènes de combats.</p>
                    <Link :href="route('mangas.search')">
                        <Button variant="destructive">Chercher un manga</Button>
                    </Link>
                </div>
