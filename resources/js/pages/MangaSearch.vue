...
const addManga = (manga) => {
    ...
    router.post(route('mangas.store'), {
        ...
        preserveState: true,
        onSuccess: () => {
            toastMessage.value = manga.title + ' a été ajouté avec succès à ta collection !';
            toastType.value = 'success';
            showToast.value = true;
            setTimeout(() => showToast.value = false, 3000);
        },
        onError: (errors) => {
            toastMessage.value = 'Oups : ' + Object.values(errors).join(' | ');
            toastType.value = 'error';
            showToast.value = true;
            setTimeout(() => showToast.value = false, 4000); // Reste 1 seconde de plus pour avoir le temps de lire
...
<template>
    ...
    <AppLayout>
        <div class="min-h-screen flex flex-col font-sans">
            ...
            <div class="flex-1 bg-[#8c8c8c] p-8 relative">
            ...
            <transition name="fade">
                <div v-if="showToast" 
                     ...
                     class="fixed bottom-5 right-5 text-white px-6 py-3 rounded shadow-lg z-50 flex items-center gap-3">
