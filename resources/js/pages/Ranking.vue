...
onMounted(() => {
    if (props.animes) {
        props.animes.forEach(anime => {
            ...
            const s = anime.pivot.status;

            if (s === 'dropped') {
                droppedList.value.push(anime);
                return;
            }
            if (s === 'watching' || s === 'plan_to_watch') {
                progressList.value.push(anime);
                return;
            }
...
const [droppedRef, droppedAnimes] = useDragAndDrop(droppedList, config(null, () => droppedAnimes.value));
const [progressRef, progressAnimes] = useDragAndDrop(progressList, config(null, () => progressAnimes.value));
</script>
...
<template>
    ...
    <AppLayout>
        ...
        <div class="py-12 bg-gray-100 min-h-screen">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-8 select-none">

                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">

                    <div class="bg-blue-50/50 rounded-xl border-2 border-dashed border-blue-200 p-4">
                        <h3
                            class="font-bold text-blue-600 mb-3 flex items-center gap-2 uppercase text-sm tracking-wide">
                    ...
                    <div class="bg-gray-200/50 rounded-xl border-2 border-dashed border-gray-300 p-4">
                        <h3
                            class="font-bold text-gray-500 mb-3 flex items-center gap-2 uppercase text-sm tracking-wide">
                ...
                <div class="mt-12">
                    <h3 class="font-bold text-gray-500 mb-2 uppercase tracking-wide text-sm">
